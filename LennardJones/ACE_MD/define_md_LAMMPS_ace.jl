...
function run_md(Tend::Int, save_dir::String; seed = 1, Temp = 0.5, dt = 0.001, dT = 10)
    ...
    d = LMP(["-screen", "none"]) do lmp
        for i = 1
            ...
            command(lmp, "log $(save_dir_seed)log.lammps")
            ...
            command(lmp, "fix vir all ave/time 1 $dT $dT c_S[1] c_S[2] c_S[3] c_S[6] c_S[5] c_S[4] file $(save_dir_seed)tmp.virial")
            command(lmp, "compute rdf all rdf 100")      # radial distribution function 
            command(lmp, "fix frdf all ave/time $dT $(Int(Tend/dT)-1) $Tend c_rdf[*] file $(save_dir_seed)tmp.rdf mode vector")

            command(lmp, "compute pe all pe")                       # potential energy
            command(lmp, "fix fpe all ave/time 1 $dT $dT c_pe file $(save_dir_seed)tmp.pe")
            
            command(lmp, "compute msd all msd com yes")
            command(lmp, "fix fmsd all ave/time 1 $dT $dT c_msd[4] file $(save_dir_seed)tmp.msd")
            
            command(lmp, "thermo $dT")
            command(lmp, "fix l0 all nve langevin $Temp $Temp 1 $seed")

            command(lmp, "timestep $dt")
            command(lmp, "dump 1 all xyz $(dT) $(save_dir_seed)dump.xyz")
            command(lmp, "dump 2 all custom $(dT) $(save_dir_seed)dump.positions_and_forces type id x y z fx fy fz")
